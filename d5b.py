...
class Line:
    ...
    def direction(self) -> Direction:
        return (
            ...
            else Direction.diagonal_up
            if self.a.x - self.b.x == self.a.y - self.b.y
            else Direction.diagonal_down
        )
    ...
    def all_points(self) -> List[Point]:
        ...
        if self.direction == Direction.diagonal_up:
            return [
                Point(self.a.x + x, self.a.y + x)
                for x in range(0, self.b.x - self.a.x + 1)
            ]
        if self.direction == Direction.diagonal_down:
            return [
                Point(self.a.x + x, self.a.y - x)
                for x in range(0, self.b.x - self.a.x + 1)
            ]
